...
public class AppRunner implements ApplicationRunner {
	...
	@Autowired RistoranteService serviceRistorante;
	@Autowired ProdottoService serviceProdotto;
	@Autowired PiattoService servicePiatto;
	@Autowired BevandaService serviceBevanda;
	@Autowired ChefService serviceChef;
	
	@Autowired ObjectProvider<Ristorante> ristorante;
	@Autowired ObjectProvider<Chef> chef;
	@Autowired ObjectProvider<Piatto> piatto;
	...
	public void creaDb() {
		if(serviceRistorante.cercaTutti().size() == 0 && 
				serviceChef.cercaTutti().size() == 0 &&
		...
		Ristorante r3 = serviceRistorante.cercaPerId(3l);
			...
			serviceChef.salva(chef.getObject("Giancarlo Esposito", CategoriaChef.GENERALE, serviceRistorante.cercaPerId(1l)));
			serviceChef.salva(chef.getObject("Walter White", CategoriaChef.GENERALE, serviceRistorante.cercaPerId(2l)));
			serviceChef.salva(chef.getObject("Jesse Pinkman", CategoriaChef.GENERALE, serviceRistorante.cercaPerId(3l)));
